...
const task1=async(req,res)=>{
    try{
        const result = await userModel.find({income:{$lt:5},$or: [
            { car: "BMW" },
            { car: "Mercedes-Benz" }
          ]});
...
const task5=async(req,res)=>{
   try{
        const result=await  userModel.aggregate([
            { $group: { _id: "$city", total_users: { $sum: 1 }, total_income: { $sum: "$income" } } },
            { $sort: { total_users: -1 } },
            { $limit: 10 },
            { $project: { _id: 1, average_income: { $divide: ["$total_income", "$total_users"] } } }
        ]);
        return res.status(200).json({status:'Success',result:result});
   }
   catch(error){
